...
if __name__ == "__main__":
    ...
    class Neighbour2Neighbour():
        ...
        def train(self, imgin):
            ...
            for epoch in range(1,101):
                ...
                if epoch % 20 == 0:
                    cv2.imwrite(str(epoch)+'.jpg', ((maxer*X.detach().cpu().numpy())+minner)[0,0,:,:])
            cv2.imwrite(outdir+'/'+file, ((maxer*X.detach().cpu().numpy())+minner)[0,0,:,:])
    ...
    for file in os.listdir(folder):
        ...
        print(file)
        image = cv2.imread(folder+'/'+file, cv2.IMREAD_UNCHANGED)
        minner = np.amin(image)
        image = image-minner
        maxer = np.amax(image)
        image = image/maxer
